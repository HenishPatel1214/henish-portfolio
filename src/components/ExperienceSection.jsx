...
export default function ExperienceSection({ experiences }) {
  return (
    ...
    >
      ...
      <div className="relative mt-12 space-y-6 border-l border-white/10 pl-7 md:pl-10">
        {experiences.map((job) => (
          <article
            key={`${job.company}-${job.role}`}
            className="relative rounded-2xl border border-white/10 bg-slate-900/45 p-6 shadow-soft"
          >
            ...
            <h3 className="mt-3 font-display text-xl font-semibold text-white">{job.role}</h3>
            <p className="mt-1 text-sm text-brand-100">{job.company}</p>
            <p className="mt-4 text-sm leading-relaxed text-slate-300">{job.summary}</p>

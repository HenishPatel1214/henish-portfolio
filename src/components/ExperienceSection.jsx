      ...
      <div className="relative mt-12 space-y-6 border-l border-[#3d63a7]/55 pl-7 md:pl-10">
        {experiences.map((job) => (
          ...
          >
            ...
            <h3 className="mt-3 font-display text-xl font-semibold text-brand-50">{job.role}</h3>
            <p className="mt-1 text-sm text-brand-100">{job.company}</p>
            <p className="mt-4 text-sm leading-relaxed text-brand-200/90">{job.summary}</p>

            <ul className="mt-5 space-y-2">
              {job.highlights.map((point) => (
                <li key={point} className="flex gap-2 text-sm leading-relaxed text-brand-200/90">
